...
async def get_semantic_graph(
    ...
    from backend.services.payload_governor import PayloadGovernor
    from backend.services.graph_metrics_service import GraphMetricsService
    ...
    PayloadGovernor.MAX_NODES = max_nodes
    PayloadGovernor.MAX_EDGES = max_edges
    governed = PayloadGovernor.govern_payload(view.get("nodes", []), view.get("edges", []))
    
    metrics = GraphMetricsService.get_graph_metrics(
        nodes=governed["nodes"],
        edges=governed["edges"],
        aggregation_enabled=aggregation,
        truncated=governed["metadata"]["truncated"],
        payload_size_bytes=governed["metadata"]["payload_size_bytes"]
    )
    ...
    metadata = {
        ...
        "entity_id": entity_id,
        "depth": depth,
        "warning": view.get("warning", False) or governed["metadata"]["truncated"],
